    ...
    def cleaning_missing_display(self, input_vars=[] ):
        ...
        for column in self.select_dtypes(include=['object']).dtypes.index:
            if self[column].nunique() < 25:
                hist_plots.append(self[column].value_counts().sort_index().plot.bar(title=column, xlabel=column,ylabel='counts', width=800, height=350)) 
        ...
        p = (null_table.opts(height=100) + typ_table.opts(width=800, height=400) + histplots ).cols(1)
        return p
